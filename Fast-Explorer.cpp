...
const float cost_per_km = 20.0;    // cost per kilometer 20 set

///////////////////////////////////////// Haversine /////////////////////////////////////////

// degrees to radians
float deg2rad(float deg) { // O(1)
    return deg * (M_PI / 180.0);
}

// since earth isn't flat, it's a sphere (oblate spheroid)
// haversine formula to calculate the distance between two cities
float haversine(float lat1, float lon1, float lat2, float lon2) { // O(1)
...
class Node {
    public:
        ...
        Node() { latitude = 0; longitude = 0; next = NULL; }
...
class MinHeapNode {
    public:
        int CityIndex;     // index of the city in the graph
        float distance;    // tentative distance of the city from the source

        MinHeapNode(int index = -1, float dist = INT_MAX) {
            CityIndex = index;
            distance = dist;
        }
};
...
class Dijkstra {
    private:
        static void calculateShortestPath(Vertex* source) {
        ...
        static vector<Vertex*> getPath(Vertex* destination) {
            vector<Vertex*> paths = destination->shortestPath;
            paths.push_back(destination);
            return paths;
        }

        static void evaluateDistanceAndPath(Vertex* adjacentVertex, Vertex* sourceVertex, float edgeDistance) {
    ...
    public:
        static vector<Vertex*> getShortestPath(Vertex* from, Vertex* to) {
...
class Student : public Passenger {
    public:
        Student(int ID, string name, char passenger_type) : Passenger(ID, name, passenger_type) {
        }
...
class Adult : public Passenger {
    public:
        Adult(int ID, string name, char passenger_type) : Passenger(ID, name, passenger_type) {
        }
...
class Elderly : public Passenger {
    public:
        Elderly(int ID, string name, char passenger_type) : Passenger(ID, name, passenger_type) {
        }
...
class Standard : public Vehicle {
    public:
        Standard() : Vehicle(50) {
         cout << "Cost per KM = 50" << endl;
        }
};

class Luxury : public Vehicle {
    public:
        void cost_cal (Passenger* p, float distance) {

            float cost = distance * 20;

            if (p->passenger_type == 'S' || p->passenger_type == 's')
                cost = cost - (cost * (20.0 / 100.0));
            
            else if (p->passenger_type == 'E' || p->passenger_type == 'e')
                cost = cost - (cost * (10.0 / 100.0));

            cout << "Total cost = Rs. " << cost << endl;
            cout << "Distance to be travelled = " << distance << " km" << endl;
        }
};
    ...
    while (true) {
        ...
        int choice ;
			  ...
			  switch(choice){
				  ...
				  case 3:{
                    cout << "SHORTEST PATH BETWEEN 2 CITIES" << endl << endl;
                    int ID = 1;
                    string name;
                    char passenger_type;
                    double distance;
